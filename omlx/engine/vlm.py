...
class VLMBatchedEngine(BaseEngine):
    ...
    async def generate(
        ...
        **kwargs,
        ...
        # Save-handle: forward the per-request handle metadata so the engine can
        # resolve save-by-handle and enforce restore isolation. Kept in its own
        # dict rather than folded into specprefill_kwargs so the shared
        # SpecPrefill helper's contract stays SpecPrefill-only.
        request_kwargs = {}
        if kwargs.get("x_omlx_request_handle") is not None:
            request_kwargs["x_omlx_request_handle"] = kwargs.get(
        ...
        return GenerationOutput(
            ...
            prompt_token_ids=(
                ...
                if getattr(output, "prompt_token_ids", None)
                else []
            ),
        )
    ...
    async def stream_generate(
        ...
        **kwargs,
        ...
        # Save-handle: forward the per-request handle metadata so the streaming
        # path resolves save-by-handle and enforces restore isolation exactly
        # like the non-streaming ``generate`` path above.
        request_kwargs = {}
        if kwargs.get("x_omlx_request_handle") is not None:
            request_kwargs["x_omlx_request_handle"] = kwargs.get(
                "x_omlx_request_handle"
    ...
    async def preflight_chat(
        ...
        **kwargs,
        ...
        # ``_process_chat_messages``).  NOTE: this peeked stream is NOT
        # the stream the scheduler block-hashes at execution time — the
        # real path goes through ``_process_chat_messages`` →
        # ``_apply_ocr_prompt`` → ``_prepare_vision_inputs``, which
        # applies the VLM template WITH per-message image placeholder
        # tokens.  Preflight therefore prices cold (no cached-prefix
        # credit) so a credited prefix that the scheduler never reuses
        # cannot under-charge the guard.
