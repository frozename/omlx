...
class Scheduler:
    ...
    # The four slot-runtime getters below read their injected hook
    # defensively: a Scheduler built via ``__new__`` (test fixtures that
    # bypass ``__init__``) has no injector attributes at all. A missing
    # attribute is treated exactly like an un-injected ``None``, so the
    # server-import fallback still runs and behaviour is unchanged
    # whenever the attribute is present.
    def _get_slot_entry(self, model_id: str) -> Any:
        lookup_fn = getattr(self, "_slot_lookup_fn", None)
        if lookup_fn is not None:
            return lookup_fn(model_id)
    ...
    def _get_slot_ctx_size(self, model_id: str) -> int:
        ctx_size_fn = getattr(self, "_slot_ctx_size_fn", None)
        if ctx_size_fn is not None:
            return int(ctx_size_fn(model_id))
    ...
    def _get_one_shot_bind_table(self) -> OneShotBindTableProtocol | None:
        table_getter = getattr(self, "_one_shot_bind_table_getter", None)
        if table_getter is not None:
            return table_getter()
    ...
    def _get_default_model(self) -> str | None:
        model_getter = getattr(self, "_default_model_getter", None)
