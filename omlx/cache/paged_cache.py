...
class PagedCacheManager(CacheManager):
    ...
    def peek_cached_prefix_tokens(
        ...
        *,
        ...
        Read-only admission probe. Unlike get_computed_blocks it performs no
        lazy SSD registration, and unlike find_shared_prefix it builds no
        block table and takes no references: it only reads the hash map (and
        asks the SSD manager whether a hash exists), so it is safe to call
        from the HTTP thread while the scheduler owns the cache. Hit/miss
        stats are deliberately left alone -- they describe the scheduler's
        own reuse, not route-time curiosity.
